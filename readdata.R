...
setwd("C:/Users/Melanie/Documents/GitHub/Projet")
...
rbnb1 <- rbnb[,-c(1:4,12:14)]

rbnb1$neighbourhood_group <- as.factor(rbnb1$neighbourhood_group)
rbnb1$neighbourhood <- as.factor(rbnb1$neighbourhood)
rbnb1$room_type <- as.factor(rbnb1$room_type)

# separation des variables en numerique/categoriel
cont_vars1 = names(rbnb1)[sapply(rbnb1, is.numeric)]
cat_vars1 = names(rbnb1)[sapply(rbnb1, is.factor)]

rbnb_cont = rbnb1[,..cont_vars1]
rbnb_cat = rbnb1[,..cat_vars1]
