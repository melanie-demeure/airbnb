shinyServer(function(input, output, session) {
    ...
    observeEvent(input$run, {
        ...
        output$selected_var <- renderText({
            paste("Une nuit chez vous couterait",cvpredictions,"dollars.")
        })
...
# MAP
   ...
   observe (if (input$quartier == "Bronx"){ 
                output$desc<-renderText("Vous voulez etre berces par le rap et le rnb, choisissez le Bronx.")}
   )
        
    observe( if (input$quartier == "Brooklyn"){
                output$desc<-renderText("Prenez le fameux pont de Brooklyn et plongez vous dans l'ambiance branchee avec une magnifique vue sur la skyline de Manhattan ")}
            )
    observe(  if (input$quartier == "Manhattan"){
                output$desc<-renderText("Des buildings qui chatouillent le ciel aux larges pelouses de Central Park, des kilometres de musees aux enseignes dedies au capitalisme : voila ce qui vous attend a Manhattan !")}         
            )
    observe (   if (input$quartier == "Queens"){
               output$desc<-renderText("Vous cherchez la proximite avec l'un des plus grands aeroports du monde : le queens est fait pour vous.  Cosmopolite, avant-gardiste, riche culturellement et gastronomiquement, ce quartier est tres interessant !")}
    )
    observe (if (input$quartier == "Staten Island"){
               output$desc<-renderText("Souvent delaisse des touristes, ce quartier offre une experience depaysante avec son fort militaire et son jardin chinois")}
       )
    
    
    
    output$quartier1 <- renderUI({
        if (input$arr=="Bronx"){
